...
- **GPU outcome (runpod, torch 2.4.1):** the `recompile_limit (8)` / `self.layer_idx == 8` warning is **gone** and numerics are bit-identical (loss 3.7320, val 3.5924). But steady-state tok/s did **not** move (~28k both ways) — see B13 for why: with the masked helpers still eager (B11) compile has only the elementwise glue to fuse, and the real ceiling was gradient checkpointing, not the recompile. The fix is still correct and worth keeping (removes wasted recompilation + eager fallback of projections); it just isn't a throughput lever on its own. The masked helpers remain `disable`d (B11) for csa/hca; the sliding path now has a FlexAttention option (B14). **Env note:** the non-interactive `python` on runpod is a *base* env (torch 2.4.1, no flex); the project **`.venv`** (local and runpod) is **torch 2.11.0+cu128 with `flex_attention`** — always train via `.venv/bin/python`. My B13 checkpointing numbers were taken on the base 2.4.1 interpreter and should be re-measured on the venv.
...
**Fix:** `configs/ablate_2k_hybrid.yaml` → `use_gradient_checkpointing: false`, `max_micro_batch: 20` (`max_vram_fraction` kept at 0.90 — an interim 0.85 was reverted as redundant once the mb cap was in place). ~**1.8× tok/s**, stable (16 GB headroom), mathematically identical training (checkpointing only trades compute for memory). **Scope:** 2K only — re-enable checkpointing for ≥8K context where the activations no longer fit. Secondary finding: the auto-batch-finder underestimates real training memory by ~4 GB (compile workspace/fragmentation), so without checkpointing it picked an unstable mb=32 that OOM'd mid-run and triggered a torch.compile shape-recompile; capping `max_micro_batch` avoids it. **Follow-up:** the Flex 2K configs now try `max_micro_batch: 28` to use more VRAM while staying below the known-unstable 32; smoke-test this cap before a long run. **Caveat:** these numbers were measured on the base 2.4.1 interpreter; a clean re-measure on the `.venv` (2.11.0) is pending.

### B14 — FlexAttention sliding path (PERF) ✅ DONE for sliding
**What:** The `sliding` attention built an O(T²) boolean mask and ran eager (B11/B2). `torch.nn.attention.flex_attention` expresses the causal+window pattern as a block-mask and compiles to a fused Triton kernel — O(N) memory, no materialized scores.

**Implementation:** opt-in `ModelConfig.use_flex_attention` (default **False**). When on (and torch≥2.5 + Triton present + `sliding_window < T` + dropout 0), the sliding layers call `flex_attention` with a cached BlockMask; everything else is unchanged. Guarded import with eager fallback, so torch<2.5 / Windows / no-Triton keep the old path. `flex_attention` is **pre-compiled** (`torch.compile`) at import because the model-level compile doesn't recapture it through the block-mask graph break — without this it silently runs the *unfused* path (materializes O(T²) scores, uses **more** memory, cratered batch 72→19). The runtime now requires the fused/precompiled path before taking the Flex branch; otherwise `use_flex_attention: true` falls back to eager sliding instead of raw/unfused FlexAttention.

**Validation:**
- Forward == eager: CPU `4.8e-7`, local GPU (RTX 4070 Ti, torch 2.11) `4.8e-7`.
- Backward == eager: local GPU, gradients match `9.3e-9`, finite.
- Fused Triton kernel == eager (runpod GPU): fp32 `2.4e-6`, bf16 `3.9e-3` (bf16 rounding).
- Initial checkpointed smoke (runpod, compile+flex, `.venv` torch 2.11): no unfused warning, batch back to **mb=72**, loss **bit-exact to eager** (step20 3.7320, val 3.5924, step40 3.4810), **tok/s 30.5k vs eager 28.1k (~+8% at 2K)**.
...
**Created (code/docs)**
- `scripts/eval_longctx.py` — GSM8K exact-match, passkey recall, and loss-by-position eval.
